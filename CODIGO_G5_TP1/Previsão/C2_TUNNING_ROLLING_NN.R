...
# 2. Grelha de procura para MLP (rede neural)
grid <- expand.grid(
  neurons    = c(20, 30, 40),
  iters      = c(200, 300, 400),
  lag_window = c(15),
...
# 3. Tuning por série com janela deslizante (rolling window)
for (col in sales_cols) {
  cat(sprintf("\n=== Tuning Rolling MLP – Série: %s ===\n", col))
  ...
  for (i in 1:nrow(grid)) {
    ...
    for (b in 1:Runs) {
      ...
      if (length(P) != length(A) || any(is.na(P))) {
        nmae_v[b] <- NA
      } else {
        nmae_v[b] <- mmetric(A, P, metric="NMAE", val=YR)
      }
      
      if (b == Runs && length(P) == length(A) && !any(is.na(P))) {
        melhor_preds_temp <- P
  ...
  if (!is.null(melhor_cfg)) {
    ...
    # Verificar se vetores são compatíveis
    if (length(melhor_preds) == length(melhor_actuals)) {
      ...
      mgraph(
        ...
        main=paste("Melhor Rolling MLP –", col)
      )
    } else {
      cat("⚠️ Comprimento de previsão e real não batem. Gráfico e métricas ignorados.\n")
    }
  } else {
